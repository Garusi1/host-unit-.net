...
name of corse : C#
PROJECT NUMBER : 2
AUTHORS:		Salay Shalom Shuker and Shlomo Mendelowith
ID:				311600605 , 308285923
*/
...
//using System.Globalization;  // thist packge for "Calendar"
...
namespace dotNet5780_02_0605_5923
{
    class Program
    {
        ...
        private static GuestRequest CreateRandomRequest()
        {
            ...
            int numMonth = new Random(DateTime.Now.Millisecond).Next(1, 12);
            int numday = new Random(DateTime.Now.Millisecond).Next(1, 31);
            gs.EntryDate = new DateTime(GuestRequest.year, numMonth, numday);

            int numDaysTostay = new Random(DateTime.Now.Millisecond).Next(2, 10);

            gs.ReleaseDate = gs.EntryDate.AddDays(numDaysTostay);
            gs.LastNight = gs.EntryDate.AddDays(numDaysTostay-2);
            return gs;
        }
